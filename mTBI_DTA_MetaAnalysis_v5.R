# ============================================================
# mTBI Biomarkers DTA Meta-Analysis — v5
# Fix: call mada:::forest.reitsma directly (triple colon)
# ============================================================
...
fit_gfap <- reitsma(gfap, formula = cbind(tsens, tfpr) ~ 1,
...
cat(sprintf("\n--- POOLED ---\nNPV: %.3f | CT avoided: %.1f%% | Missed: %d/%d (%.1f%%)\n",
            npv, ct_red, sum(gfap$FN), sum(gfap$TP+gfap$FN),
            sum(gfap$FN)/sum(gfap$TP+gfap$FN)*100))

cat("\n\n==============================================================\n")
cat("  S100B — BIVARIATE REITSMA (3 studies, N =", sum(s100b$N), ")\n")
cat("==============================================================\n\n")

s100_ok <- FALSE
tryCatch({
  fit_s100 <- reitsma(s100b, formula = cbind(tsens, tfpr) ~ 1,
                      correction.control = "all", correction = 0.5)
  print(summary(fit_s100))
  s100_ok <- TRUE
}, error = function(e) cat("Did not converge:", conditionMessage(e), "\n"))
...
# ============================================================
# 3. FOREST PLOTS — use triple colon to access mada's internal method
# ============================================================
cat("\n\n--- Generating Plots ---\n")
...
make_forest <- function(data, fit, title, filename, col_main="steelblue") {
  ...
  for (i in 1:k) {
    ...
    points(se[i], y[i], pch=19, cex=1.5, col=col_main)
  }
  axis(2, at=y, labels=data$study, las=1, cex.axis=0.8)
  polygon(c(se_pool_ci[1],se_pool_ci[2],se_pool_ci[2],se_pool_ci[1]),
          c(0.2,0.2,k+0.8,k+0.8), col=rgb(1,0,0,0.2), border=NA)
  
  # Specificity
  plot(NA, xlim=c(0.75,1.02), ylim=c(0,k+1.5), xlab="Specificity", ylab="", yaxt="n",
  ...
  for (i in 1:k) {
    ...
    points(sp[i], y[i], pch=19, cex=1.5, col=col_main)
  }
  axis(2, at=y, labels=rep("",k), las=1)
  polygon(c(sp_pool_ci[1],sp_pool_ci[2],sp_pool_ci[2],sp_pool_ci[1]),
          c(0.2,0.2,k+0.8,k+0.8), col=rgb(1,0,0,0.2), border=NA)
  
  mtext(title, outer=TRUE, line=-2, cex=1.2, font=2)
  dev.off()
}

make_forest(gfap, fit_gfap, 
            "GFAP+UCH-L1: Paired Forest Plot (Reitsma)",
            "forest_GFAP_UCH_L1_reitsma.png")
cat("Saved: forest_GFAP_UCH_L1_reitsma.png\n")

if (s100_ok) {
  make_forest(s100b, fit_s100,
              "S100B: Paired Forest Plot (Reitsma)",
              "forest_S100B_reitsma.png")
...
plot(fit_gfap, sroclwd=2, col="blue", predict=FALSE, conf.region=TRUE,
     main="SROC Comparison: GFAP+UCH-L1 vs S100B",
     xlim=c(0,1), ylim=c(0.8,1.02))
points(fpr_g, tpr_g, pch=19, cex=sqrt(gfap$N)/25, col=rgb(0.1,0.3,0.8,0.6))
fpr_s <- 1 - s100b$TN/(s100b$TN+s100b$FP+0.5*(s100b$FN==0 & s100b$TN==0))
...
legend("bottomright", legend=c("GFAP+UCH-L1 (SROC+95%CR)","S100B"),
       col=c("blue","darkgreen"), pch=c(19,17), lty=c(1,NA), cex=0.8, bty="n")
dev.off()
...
cat(sprintf("Deeks test p = %.3f", deeks_p))
...
run_sa(rbind(gfap[,c("TP","FP","FN","TN","N")],
             gfap_sens[gfap_sens$study=="Lapic 2025",c("TP","FP","FN","TN","N")]),
       "SA1 +Lapic")

run_sa(rbind(gfap[,c("TP","FP","FN","TN","N")],
             gfap_sens[gfap_sens$study!="Welch 2025",c("TP","FP","FN","TN","N")]),
       "SA2 All-Welch")

run_sa(rbind(gfap[gfap$study!="Bazarian 2021",c("TP","FP","FN","TN","N")],
             data.frame(TP=116,FP=1066,FN=4,TN=713,N=1899)),
       "SA3 Welch/Baz")

run_sa(rbind(s100b[,c("TP","FP","FN","TN","N")],
             data.frame(TP=11,FP=108,FN=0,TN=60,N=179)),
       "SA4 S100B+Kah")
...
# ============================================================
cat("\n\n==============================================================\n")
cat("  FINAL RESULTS FOR MANUSCRIPT\n")
cat("==============================================================\n\n")

cat("GFAP+UCH-L1 (8 studies, N=4969):\n")
cat(sprintf("  Se=%.1f%% (%.1f-%.1f) Sp=%.1f%% (%.1f-%.1f) AUC=%.3f NPV=%.1f%% CT-red=%.1f%%\n\n",
...
if (s100_ok) {
  cat("S100B (3 studies, N=1873):\n")
  cat(sprintf("  Se=%.1f%% (%.1f-%.1f) Sp=%.1f%% (%.1f-%.1f) pAUC=%.3f NPV=%.1f%% CT-red=%.1f%%\n",
