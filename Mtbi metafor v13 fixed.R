#!/usr/bin/env Rscript
# =============================================================================
# mTBI Biomarker DTA Meta-Analysis - Version 12 CLEAN (metafor)
# FIX: mtext(outer=TRUE) chiamato DOPO i plot, non prima
# =============================================================================

packages <- c("metafor", "ggplot2", "gridExtra", "grid")
for (pkg in packages) {
...
GFAP_data <- data.frame(
  study = c("Oris 2024", "Bazarian 2021", "Papa 2022", "Chayoua 2024",
            "Legramante 2024", "Jones 2020", "Lagares 2024 (BRAINI)", "Puravet 2025"),
...
S100B_data <- data.frame(
  study = c("Oris 2024", "Oris 2021", "Hopman 2023",
            "Seidenfaden 2021 (PreTBI I)", "Rogan 2023 (BRAIN)", "Puravet 2026"),
...
# =============================================================================
# FOREST PLOT helper
# FIX: mtext(outer=TRUE) spostato DOPO entrambi i plot()
# =============================================================================
...
draw_panel <- function(vals, lo, hi, w, pooled, pooled_lo, pooled_hi,
  ...
  plot(0, 0, type = "n",
       xlim = c(0, 1.18), ylim = c(0.5, k + 1.5),
  ...
  # Etichette asse Y (solo pannello sinistro)
  if (show_labels) {
...
plot_forest <- function(res, col_main, filename, fig_num) {
  ...
  # N originale (senza continuity correction) da usare nei titoli
  ...
  pdf(filename, width = 14, height = max(6, k * 0.65 + 2.5))

  # ── CRITICO: oma[3] abbastanza grande per il titolo esterno ──────────────
  par(mfrow = c(1, 2),
      mar   = c(5, 12, 2, 2),
      oma   = c(0, 0, 3, 0))

  # ── Pannello Sensibilità ─────────────────────────────────────────────────
  ...
  # Statistiche eterogeneità in basso a destra del pannello
  text(1.10, 0.12,
       sprintf("I\u00b2=%.0f%%  \u03c4\u00b2=%.3f  Q=%.1f (p=%.3f)",
               res$Se$I2, res$Se$tau2, res$Se$Q, res$Se$pQ),
       adj = 1, cex = 0.65, col = "#666666", font = 3)

  # ── Pannello Specificità ─────────────────────────────────────────────────
  ...
  # ── TITOLO ESTERNO: chiamato DOPO entrambi i plot ────────────────────────
  mtext(
    sprintf("Figure %d. Forest plot: %s  (k=%d, N=%s)",
            fig_num, res$label, res$k, format(N_orig, big.mark=",")),
    side = 3, line = 1.5, outer = TRUE, font = 2, cex = 1.1
  )
  mtext(
    sprintf("Pooled: Se = %.1f%% [%.1f\u2013%.1f%%]   Sp = %.1f%% [%.1f\u2013%.1f%%]",
...
plot_forest(GFAP_res,  "#1B4F8A", "Fig2_forest_gfap_v13.pdf",  2)
plot_forest(S100B_res, "#1A6B3C", "Fig3_forest_s100b_v13.pdf", 3)

# =============================================================================
# FIGURE 4: SROC Curves
# FIX: mtext sottotitolo dopo plot(), non prima
# =============================================================================

plot_sroc <- function(gfap, s100b, filename) {

  sroc_curve <- function(data) {
    se <- data$Se; sp <- data$Sp
    D  <- log(se/(1-se)) + log(sp/(1-sp))
    S  <- log(se/(1-se)) - log(sp/(1-sp))
    w  <- 1 / ((1/(data$TP+0.5) + 1/(data$FN+0.5)) +
                (1/(data$TN+0.5) + 1/(data$FP+0.5)))
    a  <- coef(lm(D ~ S, weights = w))[1]
    t  <- seq(-4, 4, length.out = 200)
    list(fpr = 1 - plogis((a-t)/2), tpr = plogis((a+t)/2))
  }
  ...
  for (res in list(
    ...
    cv <- sroc_curve(res$r$data)
    lines(cv$fpr, cv$tpr, col = res$col, lwd = 2.5)
    ...
    # Ellisse 95% CI approssimata
    theta  <- seq(0, 2*pi, length.out = 100)
    hw_sp  <- (res$r$Sp$hi - res$r$Sp$lo) / 2
    hw_se  <- (res$r$Se$hi - res$r$Se$lo) / 2
    lines((1-res$r$Sp$est) + hw_sp*cos(theta),
          res$r$Se$est      + hw_se*sin(theta),
          col = res$col, lty = 2, lwd = 1)
  }
...
plot_sroc(GFAP_res, S100B_res, "Fig4_sroc_v13.pdf")

# =============================================================================
# FIGURE 5: Clinical Summary Panel
# FIX: mtext outer DOPO tutti i 4 panel
# =============================================================================
...
plot_clinical_summary <- function(gfap, s100b, filename) {
  ...
  rows <- list(
    c("Studies (k)",      "8",   "6"),
    c("Patients (N)",     "4,550","3,582"),
    c("CT+ events",       "345 (7.6%)","242 (6.8%)"),
    c("Sensitivity (%)",
  ...
  # ── TITOLO ESTERNO: DOPO tutti e 4 i panel ────────────────────────────────
  mtext("Figure 5.  Clinical Summary \u2014 GFAP+UCH-L1 vs S100B for mTBI Triage",
        side=3, line=1.5, outer=TRUE, font=2, cex=1.1)
  mtext("Bivariate random-effects meta-analysis (REML)  |  metafor package v4.8-0  |  R 4.x",
        side=3, line=0.3, outer=TRUE, cex=0.85)
...
plot_clinical_summary(GFAP_res, S100B_res, "Fig5_clinical_summary_v13.pdf")

# =============================================================================
# FIGURE S1: Funnel Plots
# FIX: mtext outer DOPO entrambi i funnel
# =============================================================================
...
plot_funnel(GFAP_res,  "#1B4F8A", "FigS1_funnel_gfap_v13.pdf")
plot_funnel(S100B_res, "#1A6B3C", "FigS1_funnel_s100b_v13.pdf")
...
cat("\n Files generated:\n")
for (f in c("Fig2_forest_gfap_v13.pdf","Fig3_forest_s100b_v13.pdf",
            "Fig4_sroc_v13.pdf","Fig5_clinical_summary_v13.pdf",
            "FigS1_funnel_gfap_v13.pdf","FigS1_funnel_s100b_v13.pdf"))
  cat("  ", f, "\n")